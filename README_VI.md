...
Project hiện đã giữ nguyên workflow AI News cũ, đồng thời bổ sung một lớp media platform dạng modular monolith:

- `ContentItem`: model tổng quát cho draft, bài đã lên lịch và bài đã publish.
- `MediaAsset`: lưu metadata ảnh/video cho nội dung thủ công.
- `ScheduleJob`: lưu lịch đăng theo từng content item, tách khỏi lịch chạy AI News toàn hệ thống.
- `WorkflowTemplate`: mô tả các workflow có thể tái sử dụng như `manual_input -> facebook_publish`.
- `nodes/`: chứa các module xử lý nhỏ theo interface `MediaNode`.

Admin UI hiện có thêm:

- `Content Studio`: tạo bài thủ công với title, hook, body, hashtag, source, ảnh/video URL, workflow template và lịch đăng.
- `Workflow Builder`: xem các template workflow và node module đang có.
- `Schedule Calendar`: lên lịch cho từng content item và xem queue.
- `Content Library`: quản lý draft, bài đã schedule và bài đã publish.
